...
{   
    ...
    class Program
    {
        ...
        static void OnTagsReported(ImpinjReader sender, TagReport report)
        {
            ...
            {
                ImpinjTimestamp second = tag.LastSeenTime;
                string millisecondUTC = second.ToString();
                long lastseen = Convert.ToInt64(millisecondUTC);
                DateTime timestampUTC = DateTime.UtcNow;
                DateTime timestamplocal = timestampUTC.ToLocalTime();
                Console.WriteLine("EPC : {0}, LastSeenTime : {1}, PeakRssi : {2}dBm",
                                    tag.Epc,
                                    timestamplocal,
                                    tag.PeakRssiInDbm
                                    );
